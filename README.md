...
The frontend pulls all its UI primitives from [`skateboard-ui`](https://github.com/stevederico/skateboard-ui). That package has **zero runtime dependencies**. Peers are `react` and `react-dom` (this app) plus `react-router` (installed as a peer of skateboard-ui, not listed here). Apps navigate with `useSafeNavigate()` from skateboard-ui.
